"""
Обробник команд /start та головного меню

Функції:
- Привітання нових користувачів
- Відображення головного меню
- Обробка кнопок меню
- Команда /help
"""
from aiogram import Router, F
from aiogram.filters import CommandStart, Command
from aiogram.types import Message, CallbackQuery
from aiogram.fsm.context import FSMContext
from loguru import logger

from core.keyboards.reply import get_main_menu, get_back_button
from core.keyboards.inline import get_categories_keyboard
from core.database.database import AsyncSessionLocal
from core.database.queries import create_or_update_user, get_cart_summary

# Створення роутера для цього модуля
router = Router(name="start")


# ============= КОМАНДА /START =============

@router.message(CommandStart())
async def cmd_start(message: Message, state: FSMContext):
    """
    Обробка команди /start

    1. Реєстрація або оновлення користувача в БД
    2. Очищення FSM стану (якщо був)
    3. Відображення привітального повідомлення
    4. Показ головного меню
    """
    # Очищення стану FSM
    await state.clear()

    # Робота з базою даних
    async with AsyncSessionLocal() as session:
        # Створення або оновлення користувача
        user = await create_or_update_user(
            session=session,
            user_id=message.from_user.id,
            username=message.from_user.username,
            first_name=message.from_user.first_name,
            last_name=message.from_user.last_name
        )

        logger.info(f"Користувач {user.user_id} (@{user.username}) запустив бота")

    # Формування привітального тексту
    user_name = message.from_user.first_name or "друже"

    welcome_text = (
        f"🌾 <b>Вітаємо в FERM Bot!</b>\n\n"
        f"Привіт, {user_name}! 👋\n\n"
        f"Я ваш особистий <b>агропомічник</b>, який допоможе:\n\n"
        f"🛒 <b>Підібрати товари</b>\n"
        f"   • Насіння преміум якості\n"
        f"   • Ефективні добрива\n"
        f"   • Засоби захисту рослин\n\n"
        f"🌤 <b>Отримати агропогоду</b>\n"
        f"   • Прогноз на 5 днів\n"
        f"   • Рекомендації для робіт\n"
        f"   • Попередження про ризики\n\n"
        f"💡 <b>Проконсультуватися з ШІ</b>\n"
        f"   • Підбір оптимальних рішень\n"
        f"   • Розрахунок норм застосування\n"
        f"   • Відповіді на питання\n\n"
        f"💰 <b>Знайти фінансування</b>\n"
        f"   • Актуальні гранти\n"
        f"   • Допомога в оформленні\n\n"
        f"🚜 <b>Орендувати техніку</b>\n"
        f"   • Швидка заявка\n"
        f"   • Широкий вибір\n\n"
        f"<i>Оберіть розділ у меню нижче</i> 👇"
    )

    # Відправка повідомлення з головним меню
    await message.answer(
        welcome_text,
        reply_markup=get_main_menu()
    )


# ============= КОМАНДА /HELP =============

@router.message(Command("help"))
async def cmd_help(message: Message):
    """
    Обробка команди /help

    Відображає довідкову інформацію про можливості бота
    """
    help_text = (
        "<b>ℹ️ Довідка по FERM Bot</b>\n\n"
        "<b>📋 Команди:</b>\n"
        "/start - Головне меню\n"
        "/help - Ця довідка\n"
        "/menu - Повернутися до меню\n\n"
        "<b>🛒 Як користуватися каталогом:</b>\n"
        "1. Натисніть \"🛒 Каталог товарів\"\n"
        "2. Оберіть категорію (Насіння, Добрива, ЗЗР)\n"
        "3. Оберіть підкатегорію\n"
        "4. Перегляньте товари\n"
        "5. Додайте потрібні до кошика\n\n"
        "<b>📊 Розрахунок норм:</b>\n"
        "При перегляді товару натисніть \"📊 Розрахувати норму\"\n"
        "Введіть площу вашого господарства\n"
        "Отримайте точний розрахунок кількості\n\n"
        "<b>💡 ШІ-консультант:</b>\n"
        "Поставте будь-яке питання про:\n"
        "• Вибір насіння для вашої культури\n"
        "• Підбір добрив і ЗЗР\n"
        "• Технологію вирощування\n"
        "• Боротьбу з шкідниками та хворобами\n\n"
        "<b>🌤 АгроПогода:</b>\n"
        "• Прогноз на 5 днів\n"
        "• Агрорекомендації на день\n"
        "• Підписка на щоденні розсилки\n\n"
        "<b>💰 АгроГранти:</b>\n"
        "• Перегляд актуальних програм\n"
        "• Подача заявки онлайн\n"
        "• Консультація по документах\n\n"
        "<b>🚜 АгроУклон (Оренда техніки):</b>\n"
        "• Каталог доступної техніки\n"
        "• Швидка заявка на оренду\n"
        "• Розрахунок вартості\n\n"
        "<b>📞 Підтримка:</b>\n"
        "📧 Email: admin@ferm.in.ua\n"
        "💬 Telegram: @ferm_support\n"
        "🌐 Сайт: ferm.in.ua\n\n"
        "<i>Якщо виникли питання - звертайтесь!</i>"
    )

    await message.answer(help_text)


# ============= КНОПКА "КАТАЛОГ ТОВАРІВ" =============

@router.message(F.text == "🛒 Каталог товарів")
async def show_catalog(message: Message):
    """
    Відображення каталогу товарів

    Показує категорії товарів (Насіння, Добрива, ЗЗР)
    """
    catalog_text = (
        "<b>🛒 Каталог товарів FERM</b>\n\n"
        "Оберіть категорію для перегляду товарів:\n\n"
        "🌾 <b>Насіння</b> - високоякісні сорти для всіх культур\n"
        "🧪 <b>Добрива</b> - ефективне живлення рослин\n"
        "🛡 <b>ЗЗР</b> - надійний захист від шкідників та хвороб\n"
        "🔥 <b>Акції</b> - вигідні пропозиції цього місяця"
    )

    await message.answer(
        catalog_text,
        reply_markup=get_categories_keyboard()
    )


# ============= КНОПКА "АГРОПОГОДА" =============

@router.message(F.text == "🌤 АгроПогода")
async def show_weather_menu(message: Message):
    """
    Меню АгроПогоди

    Якщо локація збережена - показує погоду
    Якщо ні - пропонує ввести місто
    """
    async with AsyncSessionLocal() as session:
        from core.database.queries import get_user
        user = await get_user(session, message.from_user.id)

        if user and user.saved_location:
            # Користувач вже вказував локацію
            weather_text = (
                f"<b>🌤 АгроПогода для {user.saved_location}</b>\n\n"
                f"<i>Завантаження даних...</i>\n\n"
                f"Скоро тут з'явиться:\n"
                f"• ☀️ Поточна погода\n"
                f"• 📅 Прогноз на 5 днів\n"
                f"• 🌾 Агрорекомендації на день\n"
                f"• 📊 Графіки температури та опадів\n\n"
                f"<i>Розділ в розробці. Потребує API ключ AccuWeather.</i>"
            )
        else:
            # Нова локація
            weather_text = (
                "<b>🌤 АгроПогода</b>\n\n"
                "Для отримання прогнозу погоди та агрорекомендацій,\n"
                "будь ласка, вкажіть ваше місто.\n\n"
                "📍 <b>Надішліть назву міста</b>\n"
                "<i>Наприклад: Київ, Львів, Одеса</i>\n\n"
                "Після збереження локації ви зможете:\n"
                "• Отримувати щоденні прогнози\n"
                "• Бачити рекомендації для робіт\n"
                "• Підписатися на сповіщення\n\n"
                "<i>Розділ в розробці. Потребує API ключ AccuWeather.</i>"
            )

    await message.answer(weather_text)


# ============= КНОПКА "КОНСУЛЬТАЦІЯ ШІ" =============

@router.message(F.text == "💡 Консультація ШІ")
async def show_consultation_menu(message: Message):
    """
    Меню ШІ-консультанта

    Пояснює як поставити питання
    """
    consultation_text = (
        "<b>💡 ШІ-Консультант FERM</b>\n\n"
        "Задайте мені будь-яке питання про:\n\n"
        "🌾 <b>Підбір насіння:</b>\n"
        "   • Який сорт обрати для мого регіону?\n"
        "   • Які терміни сівби оптимальні?\n"
        "   • Яка врожайність очікується?\n\n"
        "🧪 <b>Добрива та живлення:</b>\n"
        "   • Які добрива потрібні для моєї культури?\n"
        "   • Скільки добрив внести на 100 га?\n"
        "   • Коли краще вносити?\n\n"
        "🛡 <b>Захист рослин:</b>\n"
        "   • Як боротися з шкідниками?\n"
        "   • Які ЗЗР використати проти хвороб?\n"
        "   • Чи можна змішувати препарати?\n\n"
        "📊 <b>Розрахунки:</b>\n"
        "   • Норми висіву\n"
        "   • Норми внесення добрив\n"
        "   • Витрати препаратів\n\n"
        "<i>Напишіть ваше питання або опишіть ситуацію</i> 👇\n\n"
        "<i>Розділ в розробці. Потребує API ключ OpenAI.</i>"
    )

    await message.answer(consultation_text)


# ============= КНОПКА "АГРОГРАНТИ" =============

@router.message(F.text == "💰 АгроГранти")
async def show_grants_menu(message: Message):
    """
    Меню АгроГрантів

    Відображає опції грантової підтримки
    """
    from core.keyboards.inline import get_grants_menu_keyboard

    grants_text = (
        "<b>💰 АгроГранти - Фінансова підтримка фермерів</b>\n\n"
        "Допоможемо отримати грант на розвиток вашого господарства!\n\n"
        "<b>Що ми пропонуємо:</b>\n\n"
        "📋 <b>Подати заявку</b>\n"
        "   Швидке оформлення заявки онлайн\n\n"
        "📚 <b>Актуальні програми</b>\n"
        "   Список діючих грантових програм\n\n"
        "💼 <b>Консультація</b>\n"
        "   Допомога в підготовці документів\n\n"
        "🔔 <b>Підписка на новини</b>\n"
        "   Сповіщення про нові гранти\n\n"
        "<b>📊 Популярні напрями:</b>\n"
        "• Придбання техніки та обладнання\n"
        "• Будівництво та реконструкція\n"
        "• Закупівля насіння та добрив\n"
        "• Розвиток тваринництва\n"
        "• Органічне землеробство\n\n"
        "<i>Оберіть дію з меню нижче:</i>"
    )

    await message.answer(
        grants_text,
        reply_markup=get_grants_menu_keyboard()
    )


# ============= КНОПКА "АГРОУКЛОН" =============

@router.message(F.text == "🚜 АгроУклон")
async def show_equipment_menu(message: Message):
    """
    Меню оренди техніки (АгроУклон)

    Відображає опції оренди
    """
    from core.keyboards.inline import get_equipment_menu_keyboard

    equipment_text = (
        "<b>🚜 АгроУклон - Проста оренда техніки</b>\n\n"
        "Орендуйте сільськогосподарську техніку швидко та вигідно!\n\n"
        "<b>📦 Доступна техніка:</b>\n\n"
        "🚜 <b>Трактори</b>\n"
        "   Від 50 до 300 к.с.\n\n"
        "🌾 <b>Комбайни</b>\n"
        "   Зернозбиральні, кукурудзяні\n\n"
        "💧 <b>Обприскувачі</b>\n"
        "   Самохідні та причіпні\n\n"
        "🌱 <b>Сівалки</b>\n"
        "   Пневматичні, механічні\n\n"
        "🔧 <b>Інше обладнання</b>\n"
        "   Культиватори, плуги, борони\n\n"
        "<b>✅ Переваги:</b>\n"
        "• Перевірена техніка в справному стані\n"
        "• Гнучкі умови оренди\n"
        "• Можливість доставки\n"
        "• Технічна підтримка\n\n"
        "<i>Оберіть дію:</i>"
    )

    await message.answer(
        equipment_text,
        reply_markup=get_equipment_menu_keyboard()
    )


# ============= КНОПКА "КОШИК" =============

@router.message(F.text == "🛍 Кошик")
async def show_cart(message: Message):
    """
    Відображення кошика користувача

    Показує список товарів, суму, кнопки дій
    """
    async with AsyncSessionLocal() as session:
        # Отримати підсумок кошика
        cart_data = await get_cart_summary(session, message.from_user.id)

        if cart_data['total_items'] == 0:
            # Кошик порожній
            cart_text = (
                "<b>🛍 Ваш кошик</b>\n\n"
                "Кошик порожній 🤷‍♂️\n\n"
                "Перейдіть до <b>🛒 Каталогу товарів</b>,\n"
                "щоб додати потрібні товари."
            )
            await message.answer(cart_text)
        else:
            # Формування списку товарів
            items_text = ""
            for idx, item in enumerate(cart_data['items'], 1):
                items_text += (
                    f"{idx}. <b>{item.product_name}</b>\n"
                    f"   💰 {item.product_price} грн × {item.quantity} {item.unit}\n"
                    f"   = {item.product_price * item.quantity:.2f} грн\n\n"
                )

            cart_text = (
                f"<b>🛍 Ваш кошик</b>\n\n"
                f"{items_text}"
                f"━━━━━━━━━━━━━━━━━\n"
                f"<b>📦 Товарів:</b> {cart_data['total_items']} шт.\n"
                f"<b>💰 Загальна сума:</b> {cart_data['total_price']:.2f} грн\n\n"
                f"<i>Для завершення покупки перейдіть на сайт</i>"
            )

            from core.keyboards.inline import get_cart_actions_keyboard
            await message.answer(
                cart_text,
                reply_markup=get_cart_actions_keyboard()
            )


# ============= КНОПКА "ДОПОМОГА" =============

@router.message(F.text == "ℹ️ Допомога")
async def show_help(message: Message):
    """
    Відображення довідки (дублює /help)
    """
    await cmd_help(message)


# ============= КНОПКА "НАЗАД ДО МЕНЮ" =============

@router.message(F.text == "◀️ Назад до меню")
async def back_to_menu(message: Message, state: FSMContext):
    """
    Повернення до головного меню

    Очищає FSM стан
    """
    await state.clear()

    menu_text = (
        "<b>🏠 Головне меню</b>\n\n"
        "Оберіть розділ:"
    )

    await message.answer(
        menu_text,
        reply_markup=get_main_menu()
    )


# ============= НЕВІДОМА КОМАНДА =============

@router.message()
async def unknown_message(message: Message):
    """
    Обробка невідомих повідомлень

    Показує підказку користувачу
    """
    unknown_text = (
        "🤔 Не зрозумів вашого повідомлення.\n\n"
        "Використовуйте кнопки меню або команди:\n"
        "/start - Головне меню\n"
        "/help - Довідка\n\n"
        "Або задайте питання в розділі <b>💡 Консультація ШІ</b>"
    )

    await message.answer(unknown_text)